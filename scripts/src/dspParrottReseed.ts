...
const ROSTER_PATH = path.join(REPO_ROOT, ".local", "dsp-reseed", "roster.json");
...
async function loadFastFiles(): Promise<void> {
  const inventory = JSON.parse(
    await fs.readFile(INVENTORY_PATH, "utf8"),
  ) as Inventoried[];
...
async function printSummary() {
  ...
  `);
  console.log("");
  console.log("FINAL COUNTS at DSP Parrott (school_id=1):");
  console.table(r[0]);
}
...
// =============================================================================
async function main() {
  console.log("=== DSP Parrott reseed ===");
  await assertPreconditions();
  const roster = JSON.parse(await fs.readFile(ROSTER_PATH, "utf8")) as RosterRow[];
  console.log(`Roster from FAST: ${roster.length} students`);
...
main()
  .catch((err) => {
    console.error("dspParrottReseed FAILED:", err);
    process.exitCode = 1;
  })
  .finally(async () => {
    await pool.end();
