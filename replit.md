...
- **Student Lookup**: Quick Access sidebar search → read-only "one-stop-shop" Student Snapshot (reuses the whole-child Student Profile with every edit affordance disabled, under the Teacher Roster `PrivacyGate`). The ONE editable field is a per-student parent-facing **"Message for this week's HeartBEAT"** note that surfaces on the Friday HeartBEAT (PDF block + email block, skipped when empty). Stored on `students` (`heartbeat_note` + updated_by/at). All three lookup routes (`/api/student-lookup/search`, GET + PUT `/api/student-lookup/:id/heartbeat-note`) are visibility-scoped via the exported `getVisibleStudentIds` (teachers→own roster + trusted-adult; core team/admin/counselor→school-wide), matching the Student Profile endpoint's allowed set. **Invariant: the note read must NOT fall back to the school-only-scoped `GET /students/:id` — use the visibility-scoped lookup route or out-of-roster notes leak.**
