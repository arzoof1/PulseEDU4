...
- **School Tours (Enrollment Leads)**: Per-school public "brag page" (`/tour/:schoolId`, admin-editable: headline, intro, programs/electives/proud-of, photo URLs, accent color, contact, publish toggle) with a bilingual (EN/ES) "Request Your Tour" form (family name, phone, sibling-aware student list + incoming grade, interests, source tracking). Submissions create leads in a sales-style pipeline (New→Contacted→Scheduled→Toured→Closed with outcome Enrolled/Deciding/Chose elsewhere), each with an assignable owner, append-only event timeline (`tour_request_events`), response-time clock + >24h overdue flag, and a family auto-ack. New leads trigger email + in-app notifications to the tour-notify audience (admin / Core Team / counselors / confidential secretary, assignable per-staff via `capTourNotify` in Staff & Roles) and an app-wide red alert banner linking into the pipeline. Printable brag-sheet PDF + "Post-tour document" PDF (formerly "QR leave-behind"; lead drawer has both download + a hidden-iframe Print button) whose QR points at a post-tour survey (`/tour/survey/:token`) tied back to the lead. Outcome→enrollment conversion report with source breakdown. SMS notifications are stubbed via a reusable env-gated AWS SNS helper (`lib/sms.ts`) for later activation. Module uses `authFetch` directly (not OpenAPI codegen). Brag-page media is school-uploaded (object storage, tenant-private): admins drag-drop/tap to upload multiple photos (reorder / delete / pick-cover; index 0 = cover) which families see as a swipeable carousel (touch swipe + desktop arrows/dots); a per-school `text_placement` toggle puts headline & intro above (default) or below the photos; a per-school `header_text_color` (default white, hex-validated) sets the hero headline/subheadline/school-name font color — applied inline on the `h1`/`p`/`div` because a global `h1` CSS rule otherwise forces black text on the accent gradient; and multiple labeled flyers (PNG/JPG/PDF ≤10MB, `flyers` jsonb of `{key,label,kind}`) render in their own lower section that families tap to view/download/print. Uploads use the presigned-URL flow (`/api/storage/uploads/request-url` → PUT) and are claimed via `bindObjectToSchool` on PUT `/tours/page`; the public page never sees object keys — photos/flyers stream by index through unauthenticated routes `/api/tours/public/:schoolId/photo|flyer/:idx` (ACL-bypass justified: only published pages stream). Legacy http(s) photo URLs still render (carousel + stream pass them through). District-level branding (logo + tagline) is set ONCE by a SuperUser in the Edit-district modal and inherited identically by every school in the district (schools can't change it); six additive `districts` columns (`logo_object_key`, `tagline`, `brand_hero_top`/`_documents`/`_footer`/`_watermark`) are ALTERed in by `seedTenancy` BEFORE the district insert (the drizzle INSERT references them). Four independent placement toggles — hero-top strip (default ON), printed documents (default ON), footer band (default OFF), hero corner watermark (default OFF). Logo streams publicly by ACL-bypass (`/api/tours/public/:schoolId/district-logo`; admin preview `/api/tours/admin/district-logo`); branding also rides the `/api/superuser/overview` rollup that feeds the modal, and the brag-sheet + post-tour PDFs embed the logo (downloaded to a Buffer) + tagline only when the documents toggle is on (missing logo degrades to tagline-only).
