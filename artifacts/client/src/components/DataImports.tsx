...
export default function DataImports({
  ...
  // "Download current data" panel — collapsed by default. Reset
  // whenever the user changes data type so stale filters from a
  // different kind don't leak across.
                  ...
                  {(() => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* "Download current data" opens a small inline
                            panel below this callout. The panel lets the
                            user (a) narrow the rows by grade / date /
                            subject / note type / assessment name, and
                            (b) pick which columns to include. Required
                            columns are always re-injected on the server
                            so the round trip stays valid. */}
                        <button
                          type="button"
                        ...
                        >
                          Download current data {exportPanelOpen ? "▴" : "▾"}
                        </button>
                      </div>
                    );
                  })()}
...
// ---------------------------------------------------------------------------
// ExportPanel — collapsible filter + column picker for "Download current data".
// Renders in Step 1 (Upload) of the wizard, just under the sample-CSV
// callout. Hits GET /api/data-imports/export with query params; the
// server applies filters, projects columns (always re-injecting the
// importer's required columns), and streams a CSV. We trigger the
// download via a hidden anchor so the user stays on the page.
// ---------------------------------------------------------------------------
type ExportConfig = {
  cols: string[];
  required: string[];
...
const EXPORT_CONFIG: Record<Kind, ExportConfig> = {
  rosters: {
    cols: [
      "student_id",
      "first_name",
...
// Grade chips: K + 1..12. We render all 13 regardless of the school's
// actual grade range to avoid an extra API round-trip; un-applicable
// grades simply yield zero rows on the server.
...
function ExportPanel({
  ...
  const handleDownload = () => {
    ...
    // Only send the columns param if the user actually narrowed the
    // set — saves a query-string round-trip and keeps the URL short
    // for the common "download everything" case.
    const allCols = cfg.cols.length;
    if (includedCols.size < allCols) {
