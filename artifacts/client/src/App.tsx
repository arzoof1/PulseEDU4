...
function App() {
  ...
  const [hpReportSection, setHpReportSection] = useState<
    "hub" | "byDay" | "adminDashboard"
  >("hub");
  ...
  return (
    <div className="app-shell">
        ...
        const reports: ReportTool[] = [
          {
            ...
            desc: "Live school metrics: active passes, pass usage over time, and top destinations.",
            color: "#1d4ed8",
          },
          {
            key: "byDay",
      ...
      {hpView === "reports" && (authUser?.isAdmin || authUser?.isEseCoordinator) && hpReportSection === "adminDashboard" && (() => {
        const nowMs = now.getTime();
        const todayIso = new Date().toISOString().slice(0, 10);
        const isOverdue = (p: typeof hallPasses[number]) => {
          const start = new Date(p.createdAt).getTime();
          const end = p.endedAt ? new Date(p.endedAt).getTime() : nowMs;
          return (end - start) / 60000 > (p.maxDurationMinutes || 5);
        };
        const activePasses = hallPasses.filter((p) => p.status === "active");
        const activeCount = activePasses.length;
        const reportedToday = hallPasses.filter(
          (p) => p.createdAt.slice(0, 10) === todayIso && isOverdue(p),
        );
        const reportedStudents = Array.from(
          new Set(reportedToday.map((p) => p.studentId)),
        );
        const endedAllTime = hallPasses.filter(
          (p) => p.endedAt && p.status !== "active",
        );
        const avgPassMin =
          endedAllTime.length === 0
            ? 0
            : endedAllTime.reduce((acc, p) => {
                const start = new Date(p.createdAt).getTime();
                const end = new Date(p.endedAt as string).getTime();
                return acc + (end - start) / 60000;
              }, 0) / endedAllTime.length;
        const buckets: { hour: number; label: string; count: number }[] = [];
        for (let h = 7; h <= 16; h++) {
          const hr12 = ((h + 11) % 12) + 1;
          const ampm = h < 12 ? "AM" : "PM";
          buckets.push({ hour: h, label: `${hr12}:00 ${ampm}`, count: 0 });
        }
        for (const p of hallPasses) {
          if (p.createdAt.slice(0, 10) !== todayIso) continue;
        ...
        for (const p of hallPasses) {
          destCounts.set(
            p.destination,
            (destCounts.get(p.destination) || 0) + 1,
          );
        }
          ...
          <>
            ...
            >
              <div>
                ...
                <div style={{ opacity: 0.9, fontSize: "0.9rem", marginTop: 4 }}>
                  School-wide hall pass metrics at a glance.
                </div>
              </div>
              <div
                style={{
              ...
              >
                <div style={{ fontWeight: 600 }}>School Metrics</div>
              ...
              <div style={{ display: "grid", gap: "1rem" }}>
                  ...
                  >
                    <div
                      style={{
                        fontSize: "2.5rem",
                        fontWeight: 700,
                        color: activeCount > 0 ? "#0d9488" : "#94a3b8",
                        minWidth: 56,
                      }}
                    >
                      {activeCount}
                    ...
                    <button
                      ...
                      onClick={() => {
                        setHpView("overview");
                        setPassListView("active");
                        setPassFilter("all");
                      }}
                      style={{
                        background: activeCount > 0 ? "#0d9488" : "#a7f3d0",
                        color: activeCount > 0 ? "white" : "#065f46",
                        border: "none",
                ...
                >
                  ...
                  >
                    ...
                    <span aria-hidden style={{ color: "#1d4ed8" }}>🏁</span>
                  </div>
                  <div style={{ marginTop: "0.75rem" }}>
                    {reportedStudents.length === 0 ? (
                      ...
                      <div style={{ display: "grid", gap: 4, fontSize: "0.9rem" }}>
                        {reportedStudents.slice(0, 5).map((sid) => (
                          <div key={sid} style={{ color: "#0f172a" }}>
                            • {studentName(sid)}
                          </div>
                        ))}
                        {reportedStudents.length > 5 && (
                          <div style={{ color: "#64748b", fontSize: "0.8rem" }}>
                            +{reportedStudents.length - 5} more
                          </div>
                        )}
                      </div>
                    )}
